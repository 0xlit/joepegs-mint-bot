...
async function main() {
  ...
  const LaunchpegFACT = await new web3.eth.Contract(LaunchpegABI, LaunchpegAddress);
  ...
  let tx = LaunchpegFACT.methods
    .createFlatLaunchpeg("jtest", "jts", "0xD5Ee8bC39a8f4c8Afe398704131164fBdB660aBA", "0xD5Ee8bC39a8f4c8Afe398704131164fBdB660aBA", 100, 100, 0, 50)
    .send({from: account.address, gas: 8000000});

  tx.then(async (res) => {
    console.log(res.events.OwnershipTransferred.address);
    const peg = await new web3.eth.Contract(Launchpeg, res.events.OwnershipTransferred.address);
    ...
    await peg.methods
      .seedAllowlist(
        ["0xd5ee8bc39a8f4c8afe398704131164fbdb660aba", "0x93172582e2165392ce18bb04ec88b94851232176", "0xdb1ddd3a799cd87e8d316d85c2039a7603de2101"],
        [1, 1, 1]
      )
      .send({from: account.address, gas: 8000000});

    await peg.methods
      .initializePhases(launchtime, launchtime, launchtime + 1000, launchtime + 1000, 0, 0)
      .send({from: account.address, gas: 8000000});
  });
}
